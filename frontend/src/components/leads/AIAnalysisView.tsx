...
export function AIAnalysisView({ analysis }: AIAnalysisViewProps) {
  ...
  return (
    ...
    >
      ...
      {/* Raio-X Skill Content */}
      {(!analysis.skill_used || analysis.skill_used === 'raiox') && (
      ...
      {/* Email Skill Content */}
      {(analysis.skill_used === 'email' || analysis.email_subject) && (
        <motion.div variants={stagger} initial="hidden" animate="show" className="space-y-5">
          {/* Assunto do E-mail */}
          <motion.div variants={fadeUp} className="bg-blue-950/20 border border-blue-500/15 rounded-xl p-4 relative">
            <div className="flex items-center justify-between mb-3">
              ...
              <CopyButton text={analysis.email_subject || ''} label="Assunto" />
            </div>
            <div className="bg-black/30 rounded-lg p-3.5 border border-white/[0.04]">
          ...
          {/* Corpo do E-mail */}
          <motion.div variants={fadeUp} className="bg-black/40 border border-white/[0.06] rounded-xl p-4 relative shadow-inner">
            <div className="flex items-center justify-between mb-3">
              <div className="flex items-center gap-2">
              ...
              <CopyButton text={analysis.email_body || ''} label="E-mail" />
            </div>
            <div className="bg-white/[0.02] rounded-lg p-5 border border-white/[0.02] mt-2">
              <p className="text-sm text-gray-300 whitespace-pre-line leading-relaxed">
                {analysis.email_body}
              </p>
            </div>
      ...
      {/* Call Skill Content */}
      {(analysis.skill_used === 'call' || analysis.call_script) && (
        <motion.div variants={stagger} initial="hidden" animate="show" className="space-y-5">
          {/* Gatekeeper Bypass */}
          <motion.div variants={fadeUp} className="bg-amber-950/20 border border-amber-500/15 rounded-xl p-4 relative">
            <div className="flex items-center justify-between mb-3">
              <div className="flex items-center gap-2">
                <div className="p-1.5 bg-amber-500/10 rounded-lg">
                ...
                <span className="text-xs font-semibold text-amber-400 uppercase tracking-wider">
                  Passar pela Secretária
                </span>
              </div>
              <CopyButton text={analysis.gatekeeper_bypass || ''} label="Estratégia" />
            </div>
            <div className="bg-black/30 rounded-lg p-3.5 border border-amber-500/10">
              <p className="text-sm text-amber-100/90 whitespace-pre-line leading-relaxed italic">
                "{analysis.gatekeeper_bypass}"
              </p>
            </div>
          </motion.div>
          ...
          <motion.div variants={fadeUp} className="bg-emerald-950/20 border border-emerald-500/15 rounded-xl p-4 relative">
            <div className="flex items-center justify-between mb-3">
              ...
              <CopyButton text={analysis.call_script || ''} label="Script" />
            </div>
            <div className="bg-black/30 rounded-lg p-5 border border-white/[0.04]">
              <p className="text-sm text-gray-200 whitespace-pre-line leading-relaxed font-medium">
