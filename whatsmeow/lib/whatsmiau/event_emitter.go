...
// emitter carries pre-serialized body and secret for webhook POST (envelope + optional HMAC).
type emitter struct {
	url        string
	body       []byte
	secret     string
...
func (s *Whatsmiau) startEmitter() {
	for event := range s.emitter {
		req, err := http.NewRequestWithContext(context.Background(), http.MethodPost, event.url, bytes.NewReader(event.body))
		if err != nil {
			zap.L().Error("failed to create request", zap.Error(err))
			if s.onWebhookSent != nil {
				s.onWebhookSent(event.instanceID, event.companyID, event.eventType, event.url, event.body, 0, nil, err)
			}
			continue
		}
		...
		if resp.StatusCode != http.StatusOK {
			zap.L().Error("webhook request failed", zap.Int("status", resp.StatusCode), zap.String("url", event.url), zap.String("response", string(respBody)))
		}
		if s.onWebhookSent != nil {
			s.onWebhookSent(event.instanceID, event.companyID, event.eventType, event.url, event.body, resp.StatusCode, respBody, nil)
		}
	}
}

func webhookEventEnabled(events []string, name string) bool {
...
// EmitEnvelope builds the standard webhook envelope (id, instanceId, type, payload, createdAt), marshals to JSON, and sends to the webhook URL with optional HMAC signature.
func (s *Whatsmiau) EmitEnvelope(instanceID, companyID, eventType, url, secret string, payload interface{}) {
	env := WebhookPayload{
		ID:         uuid.New().String(),
	...
	if err != nil {
		zap.L().Error("failed to marshal webhook envelope", zap.Error(err))
		return
	}
	companyIDStr := ""
	if companyID != "" {
...
func (s *Whatsmiau) Handle(id string) whatsmeow.EventHandler {
	return func(evt any) {
		go func() {
			...
			default:
				zap.L().Warn("handler semaphore full, dropping event", zap.String("instance", id))
			}
		}()
	}
}
...
func (s *Whatsmiau) handleLoggedOut(id string) {
	// Webhook DISCONNECTED before tearing down (instance config still in Redis)
...
func (s *Whatsmiau) handleMessageEvent(id string, instance *models.Instance, e *events.Message, eventMap map[string]bool) {
	// Sempre processar para persistência do chat UI; webhook só se a instância tiver MESSAGES_UPSERT
	doWebhook := eventMap["MESSAGES_UPSERT"]
	...
	if doWebhook && instance.Webhook.Url != "" {
		companyID := ""
		if instance.CompanyID != nil {
			companyID = *instance.CompanyID
		}
		s.EmitEnvelope(instance.ID, companyID, "message", instance.Webhook.Url, instance.Webhook.Secret, messageData)
	}

	// Sempre enfileirar para persistência e WebSocket do painel de chat
	if s.chatJobChan != nil {
		job := ChatJob{Type: "message", InstanceID: id, MessageData: messageData}
		select {
		case s.chatJobChan <- job:
		default:
			zap.L().Warn("chat job queue full, dropping message job", zap.String("instance", id))
		}
	}
}

func (s *Whatsmiau) handleReceiptEvent(id string, instance *models.Instance, e *events.Receipt, eventMap map[string]bool) {
	doWebhook := eventMap["MESSAGES_UPDATE"]
	...
	if doWebhook && instance.Webhook.Url != "" {
		...
		for _, event := range data {
			s.EmitEnvelope(instance.ID, companyID, "receipt", instance.Webhook.Url, instance.Webhook.Secret, &event)
		}
	}

	// Sempre enfileirar para persistência de status e WebSocket do painel de chat
	if s.chatJobChan != nil && len(data) > 0 {
		job := ChatJob{Type: "receipt", InstanceID: id, ReceiptEvents: data}
		select {
		case s.chatJobChan <- job:
		default:
			zap.L().Warn("chat job queue full, dropping receipt job", zap.String("instance", id))
		}
	}
}
...
func (s *Whatsmiau) handleBusinessNameEvent(id string, instance *models.Instance, e *events.BusinessName, eventMap map[string]bool) {
	...
	if instance.Webhook.Url != "" {
		...
		s.EmitEnvelope(instance.ID, companyID, "contacts", instance.Webhook.Url, instance.Webhook.Secret, &WookContactUpsertData{*data})
	}
}
...
func (s *Whatsmiau) handleHistorySyncEvent(id string, instance *models.Instance, e *events.HistorySync, eventMap map[string]bool) {
	...
	if instance.Webhook.Url != "" {
		...
		s.EmitEnvelope(instance.ID, companyID, "contacts", instance.Webhook.Url, instance.Webhook.Secret, &data)
	}
}
...
func (s *Whatsmiau) convertEventMessage(id string, instance *models.Instance, evt *events.Message) *WookMessageData {
	...
	case "imageMessage":
		if img := m.GetImageMessage(); img != nil {
			raw.MediaURL, raw.Base64 = s.uploadMessageFile(ctx, instance, client, img, img.GetMimetype(), "", id, jid, key.Id)
		}
	case "audioMessage":
		if aud := m.GetAudioMessage(); aud != nil {
			raw.MediaURL, raw.Base64 = s.uploadMessageFile(ctx, instance, client, aud, aud.GetMimetype(), "", id, jid, key.Id)
		}
	case "documentMessage":
		if doc := m.GetDocumentMessage(); doc != nil {
			raw.MediaURL, raw.Base64 = s.uploadMessageFile(ctx, instance, client, doc, doc.GetMimetype(), doc.GetFileName(), id, jid, key.Id)
		}
	case "videoMessage":
		if vid := m.GetVideoMessage(); vid != nil {
			raw.MediaURL, raw.Base64 = s.uploadMessageFile(ctx, instance, client, vid, vid.GetMimetype(), "", id, jid, key.Id)
		}
	}
...
func (s *Whatsmiau) uploadMessageFile(ctx context.Context, instance *models.Instance, client *whatsmeow.Client, fileMessage whatsmeow.DownloadableMessage, mimetype, fileName, instanceID, remoteJid, messageID string) (string, string) {
	var (
		b64Result string
		urlResult string
		ext       string
	...
	if s.fileStorage != nil {
		...
		// Fallback: salvar em disco para servir via endpoint de mídia
		if _, err := tmpFile.Seek(0, io.SeekStart); err != nil {
			zap.L().Error("failed to seek for local media", zap.Error(err))
			return urlResult, b64Result
		}
		baseDir := "data/media"
		safeJid := sanitizeJidForPath(remoteJid)
		safeID := strings.ReplaceAll(strings.ReplaceAll(messageID, "/", "_"), "\\", "_")
		relPath := filepath.Join(instanceID, safeJid, safeID+"."+ext)
		fullPath := filepath.Join(baseDir, relPath)
		if err := os.MkdirAll(filepath.Dir(fullPath), 0755); err != nil {
			zap.L().Error("failed to create media dir", zap.String("path", fullPath), zap.Error(err))
			return urlResult, b64Result
		}
		dst, err := os.Create(fullPath)
		if err != nil {
			zap.L().Error("failed to create local media file", zap.String("path", fullPath), zap.Error(err))
			return urlResult, b64Result
		}
		_, err = io.Copy(dst, tmpFile)
		dst.Close()
		if err != nil {
			zap.L().Error("failed to write local media", zap.String("path", fullPath), zap.Error(err))
			os.Remove(fullPath)
			return urlResult, b64Result
		}
		urlResult = "local:" + filepath.ToSlash(relPath)
	}
...
// GetProfilePictureBytes returns the profile picture image bytes for a contact/group (by instance ID and remote JID).
// remoteJID can be "5511999999999", "5511999999999@s.whatsapp.net", or "123@g.us" for groups.
// Returns (nil, "", err) if the user has no picture or privacy restricts it.
func (s *Whatsmiau) GetProfilePictureBytes(instanceID string, remoteJID string) ([]byte, string, error) {
	remoteJID = strings.TrimSpace(remoteJID)
	if remoteJID == "" {
		return nil, "", fmt.Errorf("remote JID is required")
	}
	...
	_, b64Data, err := s.getPic(instanceID, jid)
	if err != nil || b64Data == "" {
		return nil, "", err
	}
	...
	if len(decoded) >= 4 && decoded[0] == 0x89 && decoded[1] == 0x50 && decoded[2] == 0x4E {
