...
const ChatListItem = memo(function ChatListItem({ chat, isSelected, onSelectChat, tags }: ChatListItemProps) {
  return (
    <motion.button
    ...
    >
      <div className="relative shrink-0">
        <div className={`w-12 h-12 rounded-2xl flex items-center justify-center font-bold text-lg border-2 ${
          isSelected ? 'bg-white dark:bg-gray-800 border-emerald-500/20 text-emerald-600' : 'bg-emerald-100 dark:bg-emerald-900/40 border-transparent text-emerald-600 dark:text-emerald-400'
        }`}>
      ...
      <div className="flex-1 min-w-0 text-left">
        <div className="flex items-center justify-between mb-0.5">
          <h3 className={`text-sm font-bold truncate ${isSelected ? 'text-emerald-900 dark:text-emerald-100' : 'text-gray-900 dark:text-gray-100'}`}>
