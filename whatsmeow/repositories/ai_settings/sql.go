...
func (r *SQLAISettings) GetByCompanyID(ctx context.Context, companyID string) (*models.AISettings, error) {
	query := `SELECT company_id, company_name, nicho, oferta, tom_de_voz,
		COALESCE(agent_enabled, false), COALESCE(agent_system_prompt, ''), updated_at
		FROM company_ai_settings WHERE company_id = $1`

	var s models.AISettings
	err := r.db.QueryRowContext(ctx, query, companyID).Scan(
		&s.CompanyID, &s.CompanyName, &s.Nicho, &s.Oferta, &s.TomDeVoz,
		&s.AgentEnabled, &s.AgentSystemPrompt, &s.UpdatedAt,
	)
...
func (r *SQLAISettings) Upsert(ctx context.Context, s *models.AISettings) error {
	...
	query := `INSERT INTO company_ai_settings
		(company_id, company_name, nicho, oferta, tom_de_voz, agent_enabled, agent_system_prompt, updated_at)
