...
type UpdateInstanceRequest struct {
	...
	DisplayName string `json:"displayName,omitempty"` // nome amigável da instância (opcional)
	Webhook     struct {
		Base64 bool     `json:"base64,omitempty"`
		URL    string   `json:"url,omitempty"`
		Secret *string  `json:"secret,omitempty"` // omit to keep existing; send "" to clear; send value to set
...
// InstanceUsersResponse retorna os IDs dos usuários com acesso à instância.
type InstanceUsersResponse struct {
	UserIDs []string `json:"user_ids"`
}

// SetInstanceUsersRequest body para definir usuários com acesso à instância.
type SetInstanceUsersRequest struct {
	UserIDs []string `json:"user_ids"`
}
