...
type SendReactionRequest struct {
	InstanceID string `param:"instance" json:"instanceId,omitempty"`
	Reaction   string `json:"reaction,omitempty" validate:"required,min=1"`
	Key        struct {
		RemoteJid   string `json:"remoteJid,omitempty" validate:"required"`
		Id          string `json:"id,omitempty" validate:"required"`
		FromMe      bool   `json:"fromMe,omitempty"`
		Participant string `json:"participant,omitempty"` // JID do autor em grupos (opcional)
	} `json:"key"`
}
...
type EditMessageRequest struct {
	...
	Text string `json:"text,omitempty" validate:"required,min=1"`
}
